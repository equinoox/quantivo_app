...
import { calculateSoldValue, evaluateInventoryExpression, formatQuantity, getInventoryDateKey, getProductExpressionField, hasInventoryExpression, InventoryGrouping, isValidInventoryDate, ProductField, sanitizeInventoryExpression } from "@/features/inventory/lib/inventory-calculations";
...
export function InventoryListsScreen() {
  ...
  const saveList = async () => {
    ...
    const dateKey = getInventoryDateKey(draftList.date, dateFormat);
    if (!isValidInventoryDate(draftList.date, dateFormat) || !dateKey) {
      ...
      if (session?.user.id) {
        try {
          await createInventoryActivity({
            ...
            inventoryDate: updatedList.date,
            inventoryListId: updatedList.id,
            shift: updatedList.shift,
      ...
      if (session?.user.id) {
        try {
          await createInventoryActivity({
            actorNameSnapshot: session.user.name,
            actorUserId: session.user.id,
            inventoryDate: visibleList.date,
            inventoryListId: visibleList.id,
            shift: visibleList.shift,
            type: "inventory_list_exported",
          });
        } catch (error) {
          toast.error(t("inventoryActivitySaveFailed"), error instanceof Error ? error.message : undefined);
        }
  ...
  return (
    <Screen icon={<ClipboardList color={colors.secondaryDark} size={36} />} title={t("inventoryLists")} subtitle={t("inventorySubtitle")} showBackButton refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={refreshLists} tintColor={colors.orange} />}>
