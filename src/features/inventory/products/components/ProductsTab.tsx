...
import { getProductCatalogOptions, createProduct, listProducts, softDeleteProducts, updateProduct } from "@/features/inventory/products/services/products.service";
import { CatalogItem, Product, ProductInput } from "@/features/inventory/products/types/product.types";
import { productSchema } from "@/features/inventory/products/validation/product.schemas";
...
export function ProductsTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      setIsSaving(true);
      const savedProduct = editingProduct ? await updateProduct(editingProduct.id, input) : await createProduct(input);
      try {
        await createInventoryActivity({
          actorNameSnapshot: session.user.name,
          actorUserId: session.user.id,
          productId: savedProduct.id,
          productNameSnapshot: savedProduct.name,
          type: editingProduct ? "product_updated" : "product_created",
        });
      } catch (error) {
  ...
  const handleDeleteSelected = async () => {
    if (!isAdmin || !session?.user.id || selectedIds.length === 0) return;
    ...
    try {
      ...
      try {
        await createInventoryActivity({
          actorNameSnapshot: session.user.name,
          actorUserId: session.user.id,
          productId: selectedIds.length === 1 ? selectedIds[0] : null,
          productNameSnapshot: deletedProducts.map((product) => product.name).join(", "),
  ...
  return (
    <Screen icon={<Package color={colors.secondaryDark} size={36} />} title={t("products")} subtitle={t("productsSubtitle")} showBackButton>
